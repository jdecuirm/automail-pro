...
async def test_bulk_send_dispatches_all_within_quota(campaign_id):
    ...
    try:
        with patch("app.api.campaigns.remaining_quota", new_callable=AsyncMock, return_value=10):
            with patch("app.api.campaigns.send_email_task") as mock_task:
...
async def test_bulk_send_respects_quota_limit(campaign_id):
    ...
    app.dependency_overrides[get_db] = _override_db
    try:
        with patch("app.api.campaigns.remaining_quota", new_callable=AsyncMock, return_value=2):
            with patch("app.api.campaigns.send_email_task") as mock_task:
                async with AsyncClient(
                    transport=ASGITransport(app=app), base_url="http://test"
                ) as client:
